...
`readData` 成功分支随值同步返回的、路径键控的派生 schema 切片：路径终点的值 schema 子树（ref 按名保留）+ 传递闭包别名表 + 文档注释表的相关切片。它是路径键控而非值键控——值缺席时照常返回；无 active schema、路径偏离 schema 或静态无法解析时为 `null`，且 `null` 不是读的失败（ADR-0016）。预算读下投影与值同 depth 裁剪：截断处的类型子树以投影层截断标记呈现（投影包装联合，不是 ValueSchema 语义联合的新 kind，也不是值域哨兵），标记携带成员级类型线索（ref 名优先，无 ref 名时容器 kind），别名闭包与注释切片随展开层同步收缩；width 对投影无操作（投影是类型级，无实例键）。
_Avoid_: 把投影当作 live derived schema 的共享引用（每次读都是 detached 深拷贝）、把 `null` 当读失败、向载荷混入载体结构树词汇、预算读后期望全量类型口径（截断处成员口径须再访问）
...
预算触发时（depth 耗尽或 width 超限），被裁子项的键不出现在返回值中——depth 与 width 是同一种值内形态，由截断清单的 kind 区分。depth 截断的键名由清单条目的 path 尾段承载（枚举不丢键名）；width 省略只记父路径一条含省略计数。与缺席的消歧：键缺席且不在截断清单 = 真缺席；在清单 = 被裁（"这次没取"，可再访问补全，不是"数据为空"）。消费端观感等同 `undefined`，但不在值域引入"键在值 undefined"第三态（E1 吸收纪律不变）。唯一例外：`depth: 0` 时目标节点自身以同形空容器呈现（readData 的 value 键恒在场）。
_Avoid_: 同形空占位 `{}` / `[]`（已废弃——暗示"数据为空"且对必填字段类型撒谎）、魔法哨兵键（与合法数据不可区分）、被省略的键逐条进清单（违背省略初衷）、把省略误读为数据被删除

**截断清单（truncation list）**:
预算读成功结果中恒在场的截断事实通道（无截断时为空清单）：每条携带被裁位置（path，与 readData 实参同基）、裁因（depth 耗尽 / width 超限）与省略计数（depth = 被截容器直接子项数，width = 超限子项数——不是后代总数，统计后代违背零物化承诺）。depth 条目的 path 尾段即被裁键名——键名的唯一在场位置（值内已省略）；width 条目只在父路径记一条，不逐键。清单是"这次没取"的补全地图，不是数据删除记录；其规模由预算间接约束（与返回值同量级）。
