...
`readData` 成功分支随值同步返回的、路径键控的派生 schema 切片：路径终点的值 schema 子树（ref 按名保留）+ 传递闭包别名表 + 文档注释表的相关切片。它是路径键控而非值键控——值缺席时照常返回；无 active schema、路径偏离 schema 或静态无法解析时为 `null`，且 `null` 不是读的失败（ADR-0016）。预算读下投影与值同 depth 裁剪：截断处的类型子树以显式截断节点呈现（投影是引擎自构造的派生对象，截断节点不是值域哨兵），别名闭包随展开层收缩；width 对投影无操作（投影是类型级，无实例键）。
...
预算读成功结果中恒在场的截断事实通道（无截断时为空清单）：每条携带被裁位置（path，与 readData 实参同基）、裁因（depth 耗尽 / width 超限）与省略计数。depth 条目的 path 尾段即被裁键名——键名的唯一在场位置（值内已省略）；width 条目只在父路径记一条，不逐键。清单是"这次没取"的补全地图，不是数据删除记录；其规模由预算间接约束（与返回值同量级）。
