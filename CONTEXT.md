...
**SessionHost（复制会话宿主）**:
（ADR 0032）Hub 侧复制协议 namespace 级半边：每 (连接, namespace) 一个 HubSession，承载 channel 全部状态机（OPEN 矩阵/Registry open/ReplicationSession 驱动/出站合并/收口），经 Uint8Array 帧缝与 edge 对接——入站帧已被 edge 校验 sequence、出站帧以 sequence=0 占位由 edge 盖章；authorize 不在此调用，消费 edge 传入的预授权投影。session 对象随连接存活（终态不拆）。公共工厂轨 `createHubSessionHost`（经 `src/index.ts` 导出）的 `open()` 描述子为纯 JSON（`connectionKey`/`remoteInstanceId`/`namespaceId`/edge 已结算 ok-投影/`selectedCapabilities`/可选 `connectionId`），句柄提供 `handleFrame`（字节入帧）/`onFrame`（出站 sink，同步回传被分配 wire 序）/`onSignal`/`terminateUnauthorized`/`close`；denied/throw 不过公共缝，由 edge 侧处置；内部进程内 splice（`createHubSessionSink`）仍以 `openAdmission` 拉取结局。插件轨 `createHubReplicationPlugin({ listen: false })`（**仅精确 `false`** 选择该模式）在零 listener 下发布 `ctx.nomicoreHubSessionHost`（`requireHubSessionHost`）——即本工厂面 + `status`/`stop` 生命周期包装，不再要求 `tokens`/`authorization`/`verifyToken`/`authorize` 配置（认证授权是 edge 侧职责），且**不提供** `nomicoreHubReplication`（两服务入口并存属非法形态：listen 模式恒不提供 SessionHost 服务）。**γ 异步形态（ADR 0032 附录 A4，协议 §24）**：session↔edge 缝的显式异步载体——SessionHost 可与 edge 分处不同线程，中间由宿主异步字节传输承载；出站帧携 tag 过缝、wire 序经序回执回传，流控单点在 edge（账本溢出即 1011，无拒纳/闸门/信用词汇），pacing 为推-FIFO（edge 按到达序盖章，不保持跨 session 轮转公平性）；公共面为独立的新工厂/句柄类型（β 同步冻结签名逐字保留，包导出 append-only）。
_Avoid_: session 侧重检入站 sequence、把连接级帧推入 session、session 感知 drain 窗口、引入 worker_threads/MessagePort 类型、把公共描述子喂入 authorize/transport 面、以 falsy/缺省表达免 listen（仅精确 `false` 合法）、两服务入口并存、在 γ 缝上加拒纳/闸门/信用词汇（流控是 edge 单点职责）

**序回执（sequence receipt）**:
（ADR 0032 附录 A4，协议 §24）γ 异步缝上 edge→session 的 `receipt{tag, sequence}` 消息：出站帧在 edge mux 盖章点被分配 wire 序后，按帧携带的 session 侧 tag 配对回传的**序号事实**。它是序号事实回传而非接纳控制信号——不承载 sent/deferred/rejected 判别，session 不据此做发送决策（盖章决策已在 edge 同步完成）。保序条款（回执与入站帧同道 FIFO、盖章点同步投递）使「回执恒先于引用该序的 ACK」成为结构事实，session 侧三处同步序号锚（live ACK 结算 / `bootstrapSnapshotSeq` / `ownStep2Seq`）的判别逻辑与单体同构。
_Avoid_: 把序回执当接纳/背压信号、伪造非零序号、序号分配权下放 session 侧（单点盖章纪律）、回执与入站帧分道传输（破坏保序条款）
