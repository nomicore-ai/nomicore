...
状态：已接受；2026-09-14 amendment（issue #359：决策 2/3 值内形态对账 + 决策 5 切片可见性规则，见文末「ADR 0024 amendment」节）。影响包 `@nomicore/doc-runtime`、`@nomicore/namespace-runtime`、`@nomicore/namespace-registry`、`@nomicore/vfsl`、`@nomicore/vfsl-protocol`
...
> 由文末「ADR 0024 amendment（2026-09-14，issue #359）」节修订：
> depth 耗尽与 width 超限是**两种**值内形态——depth 折叠为同形空容器（键在场、
> 截断清单条目伴随），width 才是键省略。下文「同一种形态」的表述按实现形态对账修正，
> 权威 = 文末 amendment 节第 1/2 条。
...
> 由文末「ADR 0024 amendment（2026-09-14，issue #359）」节修订：
> depth 条目 path 尾段 = 被**折叠**容器键名（该键在值内以空壳在场，条目是「空壳 = 被裁」
> 的唯一辨识，不再是「键名的唯一在场位置」）。权威 = 文末 amendment 节第 2 条。
...
> 由文末「ADR 0024 amendment（2026-09-14，issue #359）」节修订：
> 「docs/aliasDocs 注释切片对被裁路径省略」的延伸决定收窄为**可见性规则**——已渲染
> 宿主的槽位 docs 随物化的值同行，只有被截子树**闭包内部**省略。权威 = 文末
> amendment 节第 3 条。
...
- **截断标记 wire 形态冗余**（#359 登记）：预算读的投影里逐字段标记 `{ kind: 'truncated', clue: { via: 'ref', name: X } }` 高度重复，有效信息等价于「字段 → 类型名（+可选性）」。标记形态（决策 5）维持冻结，消费侧可压缩呈现；未来若需紧凑表示（如 clue 字符串简写 `ref:X`），属决策 5 冻结面修订，须单独评估。

## ADR 0024 amendment：切片可见性与值内形态对账（2026-09-14，issue #359）

授权链：issue #359（实测报告 + 方案讨论收敛）；本节修订决策 2 / 决策 3 / 决策 5 的
延伸决定。除下列明示条款外，正文其余条款维持原文效力（「备选（已否决）」节为决策时
历史记录，不随条款改写）。

1. **决策 2 修订（值内截断形态按实现对账：两种，非一种）**：depth 耗尽与 width
   超限的值内形态**不同**——
   - **depth 耗尽**：被裁**容器**子项（Y.Map / Y.Array / plain object / plain
     array）折叠为**同形空容器、键在场**（`{}` / `[]`），恒伴随一条 depth 清单条目
     （`omitted` = 被折容器 raw 直接子项数 ≥ 1；真空容器折叠不记条目、`truncated`
     不变）；**终态子项**（标量 / `Y.XmlFragment`）不耗层、原样物化。消费端辨识
     「折叠壳 = 被裁（这次没取）」靠清单条目在场，不靠键缺席；「数据为空」的折叠
     无条目，二者可区分。
   - **width 超限**：超出保留前缀的子项**键省略**（不出现在返回值中，观感等同
     `undefined`）。
   - 原文「同一种值内形态」「消费端观感等同 undefined（depth 亦然）」作废；
     「不引入同形空占位」的否决范围收窄为——**无截断事实伴随**的空壳占位（与
     真空数据不可区分）仍否决；伴随清单条目的折叠壳不是缺席占位，是 depth 裁剪的
     呈现形态。实现依据：`@nomicore/doc-runtime` 预算递归（issue #334，验收锚
     S5/S6：depth:1 下容器子项折叠 `{}` / `[]` + 条目，终态子项原样）。
2. **决策 3 修订（depth 条目 path 尾段语义）**：depth 条目 path 尾段 = 被**折叠**
   容器的键名（该键在值内以折叠空壳在场；条目是「空壳 = 被裁」的唯一辨识，不再是
   「键名的唯一在场位置」）；width 条目维持「父路径单条、不逐键罗列」。
3. **决策 5 延伸决定修订（docs/aliasDocs 切片 = 可见性规则）**：原「注释切片对被裁
   路径省略」修订为——**docs 在场 ⟺ 该语法位置在返回的预算类型树中可见**（节点
   自身被渲染，或是已渲染宿主的槽位：字段 / `<item>` / `<member N>` / Record
   `<key>`）∪ 脊柱键；**被截子树闭包内部**（别名体成员注释、被截 ref 的
   aliasDocs、被截 ref 目标的值域 docs 如 `IssueState.<member N>`）继续省略。
   动机回归 ADR-0016 三元组：值通道在展开层交付的每一行（物化标量 / 折叠空壳 /
   schema 位在场的可选缺席字段）都携带同行的行级注释——截断标记的类型线索回答
   「被截处是什么类型」，槽位 docs 回答「这一行是什么」；原延伸决定把「字段的
   **类型**在 ref 终态边界被截」连坐成「字段**槽位**被裁」，使「每事实一别名」
   风格的 schema（字段类型几乎全走 ref）在 depth:1 下批量产出「值可读、口径缺席」
   的 bare value。曾评估并否决「值域 docs 随行」（被截 enum ref 的 `<member N>`
   随交付值同行）：投影随枚举成员注释规模膨胀（单枚举 12+ 条多行注释随行），与
   预算初衷相悖——理解已交付枚举值的确切含义仍需再读一层。width 对投影无操作、
   `schema: null` 单义、无 options 逐字节现行为不动。
4. **实现落点**：`@nomicore/vfsl` `resolveSchemaAtPath` 预算游走——槽位 emit 随
   宿主渲染无条件进行（原 `isTruncated` 门控连同其环安全剥离整段移除，环终止由
   进行中集 / 完成表既有机制保证）；验收 = #359 锚点（展开别名全部字段 docs 在场
   含可选缺席字段；渲染宿主的 `<item>` / `<member N>` / `<key>` 槽位在场；被截
   宿主自身的槽位与被截 ref 闭包内部缺席；叶子整读与无预算读逐字节回归锚）。

权威 = 本 amendment 节；与正文冲突处以本节为准。
