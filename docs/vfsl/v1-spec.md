...
`typeof v === 'number' && Number.isFinite(v) && !Object.is(v, -0)`——拒绝
NaN / +Infinity / -Infinity / -0 四值；`0`、`0.0` 与一切其他有限数合法。该值域
使 `number` 的执行语义与 ADR 0008 L31 声明的「JSON-compatible plain value」值域
对齐。

- **适用面**：`number` 出现的一切值判定位置同口径——同步叶子与纯值子树（§3
  「纯值上下文」）；`validateLogicalSnapshot` 与 validate-patch 两条写路径共享
  同一判定（判定执行属语义层，见 §1 出范围声明；值域口径由本节冻结，后续变更
  受 §8 语义收窄例外约束）。种子 / 直构面不在覆盖面（ADR 0021 决策 7，由消费
  侧判据兜底）。
- **消息不冻结**：四值拒绝的消息文案不属本规格冻结面（与 §4「消息正文措辞不
  冻结」同姿势；四值细分与 -0 的 `Object.is` 识别见 ADR 0021 决策 3）。
- **文本侧**：数字字面量含可选负号与可选小数部（§2 注记 7），但 `-0` 字面量
  （含值为 -0 的小数下溢形态）解析期即拒（→ VFSL-E100，请改写为 `0`）——文本
  侧不存在表达 -0 的合法写法；数据侧的零应写 `0`，不写 `-0`。
...
例外条款（两类并列，逐一经 ADR 显式裁决，独立计数：保留名增补例外首例 = ADR 0020，issue #310；语义收窄例外首例 = ADR 0021，issue #312）：

- 保留名增补例外：新增保留名属收窄，不属「只增」；仅当满足下列全部条件时允许，且须逐一经 ADR 显式裁决：(a) 仓库内全部 schema 与已知生态使用与该名无冲突；(b) 该名是某约束构造的语义必需组成，无等价的纯拓宽写法；(c) ADR 中附冲突排查证据与收窄影响面评估。
