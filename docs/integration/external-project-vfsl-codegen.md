...
`readData` 的成功结果恒四键 `{ ok, value, schema, truncated }`（[ADR-0016](../adr/0016-readdata-semantic-schema-projection.md)，形状经 [ADR-0024](../adr/0024-readdata-shape-budget.md) 修订，交付形态经 [ADR-0027](../adr/0027-readdata-projection-text.md) 改为投影文本）——`schema` 为该路径的投影文本（`string`）或 `null`（`null` 不是读的失败：无 active schema / 路径偏离 schema / 敌意 path）；`truncated` 是布尔机器信号（无形状预算读 = `false`），截断事实的唯一载体是文本末的 ✂ 段（路径 / 裁因 / 省略计数），预算读的头行如实标注 `# readData [<path>] {depth:N[,maxChildrenPerNode:K]}`。上面的适配器刻意只收窄到 `.value`，在恒四键下 `.ok` / `.value` 逐字保持，因此**零代码变化**；需要随读语义的宿主（如 agent 消费者）可在同一适配器中同时暴露 `result.schema` 投影文本，这是加法兼容的演进，不改变本页示例。预算读的静态类型是 `DeepOptional<…>`（全字段可选）——见 typed-access skill 的预算纪律。第二参 options 的闭合形状为三键 `{ depth?, maxChildrenPerNode?, maxBytes? }`（[ADR-0031](../adr/0031-readdata-byte-budget.md)）：`maxBytes`（≥1 的有限整数 ≤ 2^53−1；0 / 负数 / 非整数 / 非有限数 / 未知键 → `READ_OPTIONS_INVALID`）是**交付总量**收/拒闸——总量 = 值通道紧凑 JSON 的 UTF-8 字节 + 投影文本 UTF-8（`schema: null` 计 0），≤ 预算原样成功（交付物与同参无预算读逐字节相同），超限**零交付**返回预算失败分支 `{ ok: false, code: 'READ_BUDGET_EXCEEDED', path, measuredBytes, message }`（恰五键，`measuredBytes` 只报合计），控制形状与物化工作量仍用 `depth` / `maxChildrenPerNode` / 窗口读。
