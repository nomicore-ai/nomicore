...
单帧路径：diff 受 `maxSyncDiffBytes` 上限；收到后在 sequencer中 apply + dirty，随后发 SYNC_APPLIED。超过 `maxSyncDiffBytes` 的 diff 在已协商 `CAP_CHUNKED_UPDATE` 的组合下不再返回 `SYNC_DIFF_TOO_LARGE`，改经下述分块路径传输（issue #295）；不 fallback bootstrap、不做无界拆分。未协商组合的超限 diff 仍返回 `SYNC_DIFF_TOO_LARGE` 既有终局——v1 组合行为逐字节不变（同版本部署假设下该组合不出现，发送端复用既有交集位做改道门为纵深防御，见 ADR 0022）。`SYNC_DIFF_TOO_LARGE` 保留于错误码注册表（append-only）：已协商组合的单帧路径不再触发本码。
...
- `maxConcurrentAssembliesPerConnection`（缺省 4，约束 ≥ 1）：连接级每入站方向并发 assembly 上界，kind 无关聚合（多 ns 聚合内存上界 = 本值 × max(`maxChunkedUpdateBytes`, `maxChunkedBootstrapBytes`, `maxChunkedSyncDiffBytes`)；超额 → `UPDATE_TRANSFER_VIOLATION`/`SNAPSHOT_TRANSFER_VIOLATION`/`SYNC_TRANSFER_VIOLATION`（按 kind），ns 级、连接不拆）；
  - **分片形态计数口径（issue #423；ADR 0032 决策 5）**：`listen: false`（SessionHost 独立进程 / worker 分片）下本键**降级为 per-session 计数**——每个 SessionHost 进程各自计数，**聚合上界 = limits 值 × worker 数**；`listen`（edge 与 session 进程内组合）形态保持 **per-connection**（edge 连接级槽位单点，`hub-edge.ts` 的 `inboundAssemblySlots`）。口径差异源自缝的跨进程化，非配置键语义变化：计数键、超额码、ns 级/连接不拆的判据全部不变，listen 模式运行时行为零变化（回归锚 = `packages/ws-replication/test/ws-replication-issue423-observer-emission-split.test.ts` EM-C6a/b）；
...
- 分块同步完备性与 observer 8 型（issue #301；ADR 0022 L78–81 + 本节 §23.1 第 29–36 型）：`chunked-snapshot-{sent,applied,acked,aborted}` 与 `chunked-sync-{sent,applied,acked,aborted}` 的发射点、R21 改道归零、成功型互斥、键集冻结与 safe-field/throw 隔离/clock 缺省整键缺失纪律；资产锚 = 验收契约 `packages/ws-replication/test/ws-replication-issue301-chunked-completeness-ac-red.test.ts`（R1/R2 成功族 + R3/R4/R5/R8 aborted 族 + R6 safe-field 全 8 型 + R7 两态 latency + N1–N9 生命周期/调度负控）、类型镜像 `packages/ws-replication/test/ws-replication-api.test-d.ts`（36 型全联合精确断言）；
- observer 发射点拆分与缺面降级（issue #423；ADR 0032 决策 5）：`update-sent` hub 侧发射点 = edge 连接级 data 帧出面（盖章 sequence 事实所有者；session 侧零发射、`chunked-update-sent` 改道归属不变），`sendQueueMs` 经缝上 append-only 发送记账投影承载（宿主直驱帧整键缺席）；连接域 8 型 / namespace 域型的发射侧归属与双向零越界；授权拒绝路径复刻事件的 `connectionId` 在场纪律；缺面 dormant（无 `bufferedAmount`/clock ⇒ 整键缺席）；observer throw 隔离在两侧成立；`maxConcurrentAssembliesPerConnection` listen 计数口径回归锚；拆分前金标（键集 + 次序 + 无重复/无缺失/无乱序）。资产锚 = 验收契约 `packages/ws-replication/test/ws-replication-issue423-observer-emission-split.test.ts`（EM-C1–C7）、定偏移布局守卫 `packages/ws-replication/test/ws-replication-issue423-update-offset-guard.test.ts`（跨 lib0 varUint 1/2/3 字节边界的载荷长度判定 vs `decodeMessage`、型门、id 窗口、D6 防御分支）；
...
| `update-sent` | hub/peer | `connectionId?`、`namespaceId`、`bytes`（出站 UPDATE 帧 payload 长度；合并帧报合并后长度）；**issue #238 追加（append-only）**：`sequence`（出站帧 envelope sequence，恒在场——合并帧 = 该合并帧的 sequence；关联粒度 = wire 帧非业务写）、`sendQueueMs?`（帧实际出队 − 帧内最旧业务项入队；发送方进程内精确——区分 sender dispatch 与线上传输；clock 注入时在场）。**issue #423 发射点注记（append-only，字段集零变化）**：hub 侧 `update-sent` 的发射点 = **edge 连接级 data 帧出面**（盖章 `sequence` 事实所有者——ADR 0032 决策 5），字段由帧字节与盖章返回值单点构型（`bytes` = 出站 UPDATE 载荷长度、`namespaceId` = 帧路由键、`sequence` = `[8..12]` 盖章序）；`sendQueueMs` 经缝上 **append-only 发送记账投影**（纯 JSON `{sendQueueMs?}`，只过差值）自 session 侧携带（采样点 = 发送调用边界，session 时钟域；clock 缺省 / 无 observer ⇒ **整键缺席**）；宿主直驱 data 帧（edge 工厂 egress，无 session 记账）⇒ `sendQueueMs` **整键缺席**（缺面 dormant，与缺 `bufferedAmount` 同纪律，非 0 / 非 `undefined` 值）；分块 transfer 的结算事实在 session（`chunked-update-sent` 不改道归属）。peer 侧不拆分、发射点不变（`update-sent{side:'peer'}` 原样） |
...
| `send-failed` | 出站编码面超限/发送异常（UPDATE/SYNC 帧） | 控制帧编码面失败 + **快照超聚合上限 `maxChunkedBootstrapBytes`（本端资源超限，非对端违例；issue #295 起已协商组合超 `maxBootstrapBytes` 已改道分块、不再是终局触发面，§8.1）** | hub：`SNAPSHOT_TRANSFER_TOO_LARGE`（sent；原 `BOOTSTRAP_TOO_LARGE` 触发面在已协商组合随 §8.1 改道失效，回归锚场景 14 由实现 ticket 改写） | 场景 14 |
...
**发射侧归属表（hub 拆分形态；issue #423，ADR 0032 决策 5）**：`listen: false`（edge /
SessionHost 独立实例化）下各型事件的发射侧 = 「拥有事实的一侧」。本表只描述 hub 侧内部分工，
**不改变任何事件型/字段/词表**（信封 `side` 仍为 hub/peer 语义，与发射侧归属正交）。

| 发射侧 | 事件型 | 说明 |
|---|---|---|
| edge（连接级） | 连接域 8 型：`connection-state-changed`、`event-loop-delay-sampled`、`connection-failed`、`auth-upgrade-rejected`、`send-paused`、`send-resumed`（hub 侧可达）+ `connection-backoff-scheduled`、`goaway-received`（peer 专属；peer 不拆分，发射点不变） | 连接级事实（HELLO/capability、水位、liveness、收口）在 edge；`auth-upgrade-rejected` 为 pre-connection 形态（§23.3：无 `connectionId` 字段——握手完成前缺席的文档化形态，非缺面） |
| edge（连接级） | `update-sent` | 出站 sequence 事件：盖章事实（`[8..12]`）在 edge mux 点，发射点 = 连接级 data 帧出面（`port.sendDataFrame`，单漏斗 ⇒ seq>0 每帧恰一）；session 侧不再发射该型。`chunked-update-sent` **不在本行**（transfer 结算事实在 session，改道归属不变） |
| edge（拒绝路径复刻） | `namespace-error{direction:sent}`（`code ∈ {NAMESPACE_UNAUTHORIZED, INTERNAL_ERROR, NAMESPACE_STATE_VIOLATION}` 的授权拒绝/throw/openAdmission 拒绝与 no-sink 合成路径）、`namespace-failed{cause:open-failed}` | ADR 0032 决策 3：未授权 OPEN 不过缝、无 session 通道 ⇒ 拒绝路径的观测信号由 edge 复刻面发射；`connectionId` 按 §23.3 在场纪律携带（值 = 连接键，握手完成后恒在场） |
| session（namespace 级） | 其余 namespace 域型：`channel-state-changed`、`bootstrap-snapshot-sent`、`bootstrap-imported`、`sync-step2-sent`、`sync-diff-applied`、`update-applied`、`update-acked`、`resync-required`、`update-dropped`、通道 FSM 内的 `namespace-error`（sent/received 族）、timer 族 `namespace-failed` cause、`chunked-update-*` / `chunked-snapshot-*` / `chunked-sync-*`、`schema-rearm-*`（peer 专属） | namespace 域事实（通道 FSM、apply/ACK、transfer 结算、schema re-arm）在 session；`update-acked{sequence}` 关联**入站** sequence，留在 session（与出站盖章序归 edge 的判据同读：「事实所有权方向」） |

**形态差异登记**：工厂/分片形态下未授权 OPEN **不产生** `channel-state-changed`（未授权
OPEN 不过缝、无 session 通道——ADR 0032 决策 3 的自然结果），故拒绝路径的事件集与单体
（listen）形态略有差异——属文档化差异，非事件缺失；本表不改变任何型的在场纪律。
...
集合的配置值，非时间戳非测量值；issue #295 起集合追加 `assemblyTimeoutMs`——
kind=1 assembly 停滞触发的 `bootstrap-timeout` 承载值，见 §23.1）；
