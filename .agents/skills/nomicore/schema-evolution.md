...
7. Coordinate Peers before enabling writers that use changed schema paths. Per ADR 0018, a Peer's replication apply slot re-arms its active schema synchronously when the replicated SCHEMA change commits; confirm convergence per Peer via the `schema-rearm-applied` observer event (or `getActiveSchema()` fingerprint) before publishing dependent Peer business capabilities. A `schema-rearm-failed` event is fatal-class: that Runtime's writes stay disabled and the standalone server logs and exits by default — treat it as an ops incident (fix version/schema, then reset/restart). Controlled reset/re-bootstrap or restart remains the documented fallback.
...
Complete only when the proposed schema and final ROOT validate, generated projections are fresh, all affected writers pass projection-aware typecheck, Hub `replaceSchema()` succeeds through a lease, Hub behavior is verified, every required Peer has converged (a `schema-rearm-applied` event or a `getActiveSchema()` fingerprint matching the Hub — reset/restart is only the fallback, not a required step), dependent business capabilities are published only afterward, and no snapshot or live Yjs object was edited directly. The full upgrade runbook and the `schema-rearm-failed` ops playbook live in `$NOMICORE_ROOT/docs/integration/schema-evolution.md`.
