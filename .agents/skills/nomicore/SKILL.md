---
name: nomicore
description: Integrate Nomicore into an independent project. Use when authoring or validating VFSL schemas, generating TypeScript namespace projections, implementing typed namespace reads/writes or change subscriptions (watchMap), composing Cordis Instance/Clock/Timer/Persistence/Registry (including namespace diagnostic change log / observability wiring) and role-specific replication plugins, or configuring standalone Hub/Peer deployment and replica recovery.
---
...
- **Typed access** — generating `generated.ts`, wiring TypeScript, implementing namespace read/write code, consuming and interpreting `readData()` results (value + projection text + `truncated`; shape-budget reads with `depth` / `maxChildrenPerNode`, the `‡` depth-fold marker, the `✂ 截断事实：` truncation-facts section, and re-read completion), selecting a meaningful slice of a wide container with window reads `readArray` / `readMap` (`n` + sort term + optional `where` equality filter — with `where` the matched total is not promised, `truncated` becomes a fill test, and the `✂` window-facts line is never assembled; entry identity back-stitching; loud failure codes), or subscribing to keyed-container change signals with `watchMap` (change notification / subscription / watch requests — predicate vocabulary `where`, `WATCH_MAP_*` establishment failures, the `data` / `invalidate-all` / `watch-end` notification kinds, `origin` self-echo suppression, and the pull-based consumption protocol): read [typed-access.md](typed-access.md).
