...
`readData(path, { depth, maxChildrenPerNode })` ([ADR 0024](../../../docs/adr/0024-readdata-shape-budget.md)) bounds how much a single read materializes: `depth` limits how many container levels below the target are expanded, `maxChildrenPerNode` keeps at most the first K children of each expanded node (map order is not a pagination promise). `depth` is ≥ 0, and `0` is legal — a **skeleton read**: the target container folds to a same-shape empty shell (`{}` / `[]`) plus one ✂ depth fact counting its direct children, materializing nothing below the target (a scalar target returns the scalar as-is, `truncated: false`; only negative or non-integer values are rejected). Omitting options renders the full projection text; invalid options (unknown keys, negative or non-integer values, non-object) fail loudly with the stable `READ_OPTIONS_INVALID` result — never silently degrade to `schema: null`. The projection text is always-on: there is no `schema` opt-in switch, and the closed options shape accepts only the three budget keys `{ depth?, maxChildrenPerNode?, maxBytes? }` (the byte-budget key added by [ADR 0031](../../../docs/adr/0031-readdata-byte-budget.md) — next subsection). Do not confuse this with window reads' `n`, which is ≥ 1 (`n: 0` is invalid there — to count, do a normal read).
...
### Byte-budget reads: `maxBytes`

`maxBytes` ([ADR 0031](../../../docs/adr/0031-readdata-byte-budget.md)) is a **delivery-total gate**, not a shape knob: it never trims, never lowers depth, and never reshapes what you get — over the budget the read delivers **nothing** and rejects loudly.

- **Domain**: a finite integer ≥ 1 and ≤ 2^53−1 (`Number.isSafeInteger`). `0`, negatives, non-integers, non-finite numbers, out-of-domain values and unknown keys fail loudly with the stable `READ_OPTIONS_INVALID` result (no new validation code); omitting the key means "no budget" — behaviour is byte-for-byte unchanged, with no magic default.
- **Measured total** = value channel + schema channel: `utf8(JSON.stringify(value))` (compact JSON, key order = delivery order; `value === undefined` counts 0) plus the UTF-8 byte length of the projection text (`schema: null` counts 0; the head line and the `✂ 截断事实：` section live inside the text and count naturally, never exempt).
- **`total ≤ maxBytes` → success, unchanged**: the delivery is byte-for-byte identical to the same read without `maxBytes` (a total of exactly `maxBytes` succeeds — the bound is `≤`; a zero total is never an overrun).
- **`total > maxBytes` → zero delivery**: the synchronous result is the failure branch `{ ok: false, code: 'READ_BUDGET_EXCEEDED', path, measuredBytes, message }` — exactly five keys, no `value` / `schema` / `truncated`; `measuredBytes` reports the combined total only (no split into channels).
- **Measured after shaping**: the gate runs on the shaped delivery, so a `depth: 1` read of a 358-byte target measures its shaped 415 bytes, not the pre-shaping number. A path whose value is absent still measures its projection text and can exceed the budget (`value === undefined` contributes 0); a `schema: null` read measures the value channel only.
- **Division of labour** (ADR 0031 decision 6): use `depth` / `maxChildrenPerNode` / window reads to control **shape and materialization work**; `maxBytes` only governs the **delivered total** — a wide read without a structural budget may materialize fully and then be rejected (the same order of cost as a caller-side cap, no worse). Retrying after `READ_BUDGET_EXCEEDED` is a deterministic caller action: lower the budget, or add a shape/window budget to shrink the delivery, then read again.

The byte budget is not a truncation mode: an over-limit read carries no ✂ facts (nothing was delivered), and the `✂ 段` is never the place to look for byte facts (the「字节预算」entry in `CONTEXT.md`).

### Window reads: `readArray` / `readMap`

When you need a *meaningful* slice of a container — the newest K entries of an append log (`readArray` from the tail), the newest K by a time `field` (the map face), a stable key-ordered window, the K tasks with the highest priority, or every entry matching an equality predicate (all tasks with `state == 'claimed'`) — a `readData` width budget will not do: `maxChildrenPerNode` is a structurally blind guard (first K in carrier order), not a selector. Window reads ([ADR 0028](../../../docs/adr/0028-window-read.md); predicate filtering via the `where` vocabulary per [ADR 0029](../../../docs/adr/0029-filtered-window-read.md)) are the value-aware selector on the lease public surface. Division of labour with `readData` budgets (the「窗口读」entry in `CONTEXT.md`): the budget's width is a **structural guard** — it never looks at values; a window read is the **value-aware selector** — it decides **which entries to read at all**, while `depth` / `maxChildrenPerNode` (the same [ADR 0024](../../../docs/adr/0024-readdata-shape-budget.md) axes) bound the shape of **each selected entry**; the target width is governed by `n`. Four typical call paths:

```ts
// Newest-K of an append log (readArray's index basis is positional — asc = the first K entries
// from [0], desc = the last K from the tail; entry values never participate in selection, so
// container/record entries window exactly as well as scalars. A log that appends newest-last
// takes its most recent K entries with desc):
const recent = lease.readArray(['workRecords'], { n: 3, orderBy: { by: 'index', dir: 'desc' }, depth: 1 })
...
// Every entry matching a predicate (filtered window, ADR 0029): where narrows the candidate
// set BEFORE ordering (pipeline where → orderBy → n), so the window is taken over the matched
// subset — a positional scan stops as soon as n matches are found:
const claimed = lease.readMap(['tasks'], { n: 50, where: [{ field: 'state', equals: 'claimed' }] })
// value: [{ key: 't1', value: … }, { key: 't3', value: … }, { key: 't5', value: … }]
// truncated: false — 3 < 50: the matched set is exhausted, there are certainly no more
// (kept === n would read true: the window is full, there may be more matches beyond it)
```

**Rules and the v1 vocabulary.** `n` is required and ≥ 1 (`n: 0` is invalid — to count entries, do a normal read; with `where`, count via a large `n`, see below); the direction always rides on the sort term (`asc` default). The vocabulary is closed: `readArray` accepts only `{ by: 'index' }` (its default), `readMap` accepts `{ by: 'key' }` (its default) or exactly one single-segment `{ field: '…' }`; anything else — a `field` term on `readArray`, `by: 'index'` on `readMap`, a multi-segment field — is rejected loudly, never reinterpreted. An optional `where` narrows the candidate set before ordering: a conjunctive array of equality predicates, each v1 term exactly `{ field, equals }` with `field` a single literal key (dots are not split) and `equals` in the closed scalar set `string | number (finite) | boolean | null`; the pipeline is **`where` → `orderBy` → `n`**, so the window is taken over the *matched* subset. That vocabulary is closed in the same way: an empty array, more than 16 terms, `in` / ranges / `OR` / `NOT` / multi-segment fields / container deep-equality, accessor or non-plain terms, and `NaN` / `±Infinity` comparisons are rejected loudly with `WINDOW_OPTIONS_INVALID`, never reinterpreted (`''`, `0`, `false`, and `null` are legal values — `null` matches an actual `null`, never a missing field). Data-side dirt is the opposite disposition: a missing field, a non-scalar or non-finite field value, or a non-drillable entry is a **quiet non-match** — dirty entries neither displace normal ones nor fail the read. What each basis orders by: the array face (`by: 'index'`) orders by **position** — the entry's own index in the current carrier (`asc` = the first K from `[0]`, `desc` = the last K from the tail; entry values never participate in selection); the key basis by the **entry key** (code points), the field basis by the **entry's field value**. The value-ordering total of the map bases is deterministic: finite numbers (numeric order) → strings (code-point order) → incomparable group (missing / non-finite / null / boolean / containers) **always last, in both directions**, so dirty entries lacking the sort basis never displace normal ones; ties anchor on the key with `asc` fixed regardless of `dir` — same data, same window. Choosing a face: an append log (newest appended last) takes newest-K directly with `readArray` + `dir: 'desc'`; value-driven selection (newest by a timestamp field, top-K by priority) needs the map face with a `field` basis — `readArray` has no value basis in v1. Indices move under prepends/deletes and replicated merges, so consume `entry.index` right after the read (identity back-stitching below) rather than caching it as a durable handle. A window read is not a pagination API: there is no offset or cursor.
...
For `readArray` the identity segment is `entry.index` — already a number, so the stitched array path keeps numeric segments (`['workRecords', entry.index]`, never `'0'`-style strings); for `readMap` it is `entry.key`, a string key as in the loop above. Every selected entry equals a same-budget `readData` of that entry's own path (the composed depth anchor), and unselected children are never materialized — never reason about entries the window ranked out, and never treat a window as a complete snapshot or a page (with `where`, the one exception below: a `kept < n` window *is* the complete matched set).

**✂ window facts.** Without `where`, `truncated === kept < total`, where `total` is the target's candidate entry count (array length with sparse holes counted; map keys whose value is not `undefined`). When `kept < total`, the text ends with one window-facts line — `- <path> · 窗口 · 基 <basis> <dir> · kept <n>/total <N>` — where `<basis>` renders as `index`, `key`, or `field:<name>` (e.g. `- workRecords · 窗口 · 基 index desc · kept 2/total 3`). Read the line as the statement of which window you hold: basis + direction + how many of how many; `total: 0` means `truncated: false` and no `✂` section. Unlike a width trim, omitted entries were **ranked out by the stated basis** — they are not "first K kept, rest unknown".

With `where`, the matched total is **never promised** (a positional scan can stop as soon as `n` matches are found, so no honest count exists; `total` has no place on the four-key surface) and the `✂` window-facts section is **never assembled** — no filtered-slot substitute is emitted, and `schema` stays the element-scope projection text. `truncated` therefore degrades to a **fill test**: `kept === n` → `true` (the window is full — there may be more matches beyond it), `kept < n` → `false` (the matched set is exhausted — certainly no more matches). So for a filtered read, `truncated: false` is the *completeness* signal, not an absence of signal; to count matches, pass a large `n` (the only count channel — a dedicated `count` remains a separate evolution slot).

**Byte budget on windows: `maxBytes`.** Window options take the **same** `maxBytes` delivery-total gate as the readData face ([ADR 0031](../../../docs/adr/0031-readdata-byte-budget.md)); the three read faces are one contract — same code, same message template, same five-key payload shape — and the face is identified by the call site, never by the text. The domain is the same: a **finite integer ≥ 1 and ≤ 2^53−1**. `0`, negatives, non-integers, non-finite numbers, out-of-domain values and a non-data (accessor) property fail loudly with the window face's own stable `WINDOW_OPTIONS_INVALID` (no new code; the message carries the `maxBytes` domain and is not the unknown-key message), and omitting the key means "no budget" — the read is byte-for-byte unchanged.

The measured total is the **entry list including its `{ index | key, value }` wrapper** as compact JSON UTF-8 plus the **element-scope projection text** UTF-8 (the `✂ 截断事实：` window-facts section and the `‡` fold footer live inside the text and count naturally, never exempt; `schema: null` counts 0). `total ≤ maxBytes` succeeds with a delivery byte-for-byte identical to the same window read without `maxBytes`; `total > maxBytes` delivers **nothing** — the synchronous failure branch `{ ok: false, code: 'READ_BUDGET_EXCEEDED', path, measuredBytes, message }`, exactly five keys with no `value` / `schema` / `truncated`, `measuredBytes` reporting the combined total only. A budget never reshapes what you get: no trimming, no depth lowering, no fitting.

Three window semantics are unchanged by a budget, and they matter most on filtered windows: an over-limit `where` window raises the **same** failure branch (a budgeted window never silently drops entries to fit); the fill test `truncated === (kept === n)` is settled before the gate and is **not** driven by the budget (byte-identical filtered windows with opposite fill verdicts keep their own verdicts); and the ✂ window-facts section stays **never assembled** under `where`, budget or not. A budgeted window read is also not a counting or pagination device — it only decides whether the whole window (plus its projection text) is deliverable at all. Ordering is fixed: the options rules are checked first, then the target/carrier, then the budget — an illegal budget never masks an absent target, and a legal budget never absorbs one. Retrying after `READ_BUDGET_EXCEEDED` is a deterministic caller action (lower the budget, or narrow `n` / add `depth` / `maxChildrenPerNode` so the delivered window plus text shrinks), same as on the readData face.
...
- `WINDOW_OPTIONS_INVALID` — the options violate the rules (`n: 0` or non-integer `n`, illegal enum, `by: 'index'` on `readMap`, a `field` term on `readArray`, malformed `orderBy`, malformed `where` — non-array, unknown term keys, empty or over-limit arrays, non-scalar or non-finite `equals`, and an out-of-domain `maxBytes`). **Fix the options** — a caller bug, not a data condition.
- `READ_BUDGET_EXCEEDED` — the shaped delivery (entry list + element-scope projection text) exceeds `maxBytes`. **Lower the budget or narrow the window/shape**, then read again: the rejection is complete and deterministic — nothing was delivered and nothing was trimmed.
