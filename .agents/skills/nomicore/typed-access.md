...
6. Keep runtime validation and static typing distinct:
   ...
   - the adapter calls public `NamespaceLease.readData()` (a successful read returns `{ ok, value, schema, truncated, truncations }` — see [Read result: value plus semantic schema projection](#read-result-value-plus-semantic-schema-projection)) and `mutateData()`;
...
Every successful `readData(path)` returns `{ ok: true, value, schema, truncated, truncations }` ([ADR 0016](../../../docs/adr/0016-readdata-semantic-schema-projection.md), shape revised by [ADR 0024](../../../docs/adr/0024-readdata-shape-budget.md); see the 语义 schema 投影 entry in `CONTEXT.md`): besides the plain logical value, `schema` carries the path's semantic schema projection — the semantics an agent consumer needs to interpret the value (value domains, literal unions, constraints) and to prepare a follow-up write. `truncated` / `truncations` are always present (`false` / empty array for a full read) and report shape-budget truncation facts.
...
`readData(path, { depth, maxChildrenPerNode })` ([ADR 0024](../../../docs/adr/0024-readdata-shape-budget.md)) bounds how much a single read materializes: `depth` limits how many container levels below the target are expanded, `maxChildrenPerNode` keeps at most the first K children of each expanded node (map order is not a pagination promise). Omitting options returns the full projection byte-for-byte; invalid options (unknown keys, negative or non-integer values, non-object) fail loudly with the stable `READ_OPTIONS_INVALID` result — never silently degrade to `schema: null`.
...
The in-value truncation form depends on the cause ([ADR 0024 #359 amendment](../../../docs/adr/0024-readdata-shape-budget.md)): when `depth` runs out, cut **container** children fold to same-shaped empty containers with the key present (`{}` / `[]`) plus one `truncations` entry each (`omitted` = the folded container's direct child count; a genuinely empty container folds with no entry); terminal children (scalars, semantic strings) ride through untouched. When `width` overruns, keys beyond the kept prefix are omitted from `value` (never `{}` placeholders or `undefined`-valued keys) with one parent-path entry. Disambiguation: a folded shell **listed** in `truncations` was cut ("not fetched this time"); a shell with no entry is genuinely empty data; a key absent from the value and absent from `truncations` is truly absent. The schema projection is truncated at the same depth, with truncation markers (`kind: 'truncated'` carrying a ref-name or container-kind clue) so the next drill-down path can still be constructed. `docs`/`aliasDocs` slices follow visibility: doc rows for slots of rendered hosts (fields, `<item>`, `<member N>`, `<key>`) ride with the delivered rows — field-level docs accompany materialized values; docs inside a cut subtree's closure (alias-internal member docs, the cut ref's `aliasDocs`, enum value-domain docs like `IssueState.<member N>`) stay omitted — interpreting a delivered enum value exactly still needs one shallower read. The value + schema + 数据口径 triad applies within what the budget returned. `maxChildrenPerNode` does not affect the projection.
