...
- `WINDOW_OPTIONS_INVALID` — the options violate the rules (`n: 0` or non-integer `n`, illegal enum, `by: 'index'` on `readMap`, a `field` term on `readArray`, malformed `orderBy`, malformed `where` — non-array, unknown term keys, empty or over-limit arrays, non-scalar or non-finite `equals`). **Fix the options** — a caller bug, not a data condition.
