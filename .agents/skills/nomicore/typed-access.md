...
Truncated keys are omitted from `value` (never `{}` placeholders or `undefined`-valued keys); the `truncations` list states each cut (`path` — its tail segment is the omitted key's name for `depth` entries — `kind: 'depth' | 'width'`, `omitted` count). Disambiguation: a key absent from the value **and** absent from `truncations` is truly absent; a key absent but listed was cut — read one level shallower to enumerate it. The schema projection is truncated at the same depth, with truncation markers (`kind: 'truncated'` carrying a ref-name or container-kind clue) so the next drill-down path can still be constructed, and `docs`/`aliasDocs` slices omit cut paths — the value + schema + 数据口径 triad applies within what the budget returned. `maxChildrenPerNode` does not affect the projection.
