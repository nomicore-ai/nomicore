...
6. `$NOMICORE_ROOT/docs/adr/0032-transport-decoupling-edge-session-split.md` — edge/session split contract, when the host owns the transport edge or runs the `listen: false` SessionHost mode.
...
- Use the edge/session split (ADR 0032) when the host owns the transport edge itself — for example sharding namespaces across worker threads behind one socket-holding ingress. `createHubReplicationEdge()` is a plain factory, not a Cordis plugin, with no Registry dependency: it accepts already-upgraded sockets through `accept(transport, { token })` (runs the injected `verifyToken` exactly once) or `acceptTrusted(transport, identity)` (pre-verified identity), assigns a connection handle per accept, and runs HELLO/capability negotiation, liveness, connection-level backpressure, and the OPEN admission pipeline (bounded pending buffer, concurrent-OPEN ceiling, loud connection close on sink-resolution failure). After authorization it resolves each (connection, namespace) session sink through the host's `resolveSessionSink(connectionKey, namespaceId, authorization)` callback; pair it with `createHubSessionHost()` on the namespace-owning side for the same-process synchronous seam, or with `createHubAsyncSessionHost()` when the SessionHost runs on a separate thread (γ asynchronous seam, ADR 0032 appendix A4; protocol §24). A Cordis host that owns no listener instead installs `createHubReplicationPlugin({ listen: false })` (exact `false` selects this mode) and discovers `requireHubSessionHost(ctx)`; that mode publishes no `nomicoreHubReplication` service and consumes no `tokens`/`authorization`/`verifyToken`/`authorize` config — authentication and authorization are edge-side duties. The seam carries only namespace-domain frames as `Uint8Array` plus pure-JSON control signals; never reimplement the connection-level protocol host-side, which forks the state machine. On the γ seam, outbound frames carry a session-allocated `tag` and the wire sequence returns as an edge→session `receipt{tag, sequence}`; the host owns a dedicated FIFO channel pair per (connectionKey, namespaceId), bridges the edge stamp return value back as a receipt in the same synchronous segment at the stamp point, and should set `asyncDataAdmissionFatal: true` on the edge factory — omitting it silently keeps β flow-control semantics (ns-level send-failed resync, connection alive) with zero diagnostics.
...
- chunked transfers (ADR 0013/0022): `chunked-update-sent`/`chunked-update-applied`/`chunked-update-acked`/`chunked-update-aborted`, `chunked-snapshot-*` and `chunked-sync-*` in the same four-type shape—on negotiated connections a chunked transfer settles through these instead of the plain-family events (`update-*`, `bootstrap-imported`, `sync-step2-sent`), so persist both families;
...
For `resync-required{cause:"send-failed"}`, read the appended `reason` before choosing a fix: `send-frame-rejected` means inspect the connection/backpressure state machine using the attached `channelState`/`connectionState`/queue/in-flight counts and optional `bufferedAmount`. `update-too-large` (with `updateBytes`/`maxUpdateBytes`) now fires only when the update is not chunkable—either the connection never negotiated `CAP_CHUNKED_UPDATE` or the update exceeds `maxChunkedUpdateBytes` (default 4 MiB)—so first check the peer runs a version that negotiates the capability, then shrink the business update or raise `maxChunkedUpdateBytes` (`maxUpdateBytes` is only the per-chunk size). On negotiated connections, updates above `maxUpdateBytes` up to `maxChunkedUpdateBytes` stream as UPDATE_CHUNK sequences and settle through `chunked-update-sent`/`chunked-update-applied`/`chunked-update-acked` with no drop and no resync. Other `cause` values carry no `reason` field. An oversized non-chunkable update dropped while the send queue is non-empty (F4 liveness, no resync declared) reports `update-dropped{reason:"update-too-large"}` with the same safe numeric context instead—counting invariant: exactly one observer event per oversized drop, and repair comes from the next reconciliation round.

Chunked transfer stalls are non-terminal except on bootstrap: a live-update or sync-diff assembly that stops making progress past `assemblyTimeoutMs` (default 30 s, sliding deadline reset by every received chunk) drops the partial on the receiving side with `chunked-update-aborted`/`chunked-sync-aborted` (reason from the closed `ChunkedUpdateAbortReason` set) and declares `RESYNC_REQUIRED` with reason code `UPDATE_TRANSFER_EXPIRED` or `SYNC_TRANSFER_EXPIRED`; the peer recovers through an ordinary sync round. A stalled snapshot assembly instead ends the namespace in the `BOOTSTRAP_FAILED`/`SNAPSHOT_TRANSFER_*` family (terminal `failed`). Receive-path transfer faults are also terminal: `*_TRANSFER_VIOLATION` (not retryable—wire-level contract breach) and `*_TRANSFER_TOO_LARGE` (retryable after raising the matching `maxChunked*Bytes` limit) both end the namespace `failed` while the connection stays ready.
