...
These factories remain public for trusted hosts that own their own transport integration; they consume public Registry leases and `ReplicationSession` and never expose live `Y.Doc`, Persistence handles, or Registry internals. `createHubReplication()` and `createPeerReplication()` are the monolithic controllers. The Hub edge/session split ([ADR 0032](../../docs/adr/0032-transport-decoupling-edge-session-split.md)) additionally exposes three append-only seam factories: `createHubReplicationEdge()` (a plain factory, not a Cordis plugin, no Registry dependency) for the socket-holding connection half, `createHubSessionHost()` for the same-process synchronous session half, and `createHubAsyncSessionHost()` for the γ asynchronous seam where the SessionHost runs on a separate thread (ADR 0032 appendix A4; [protocol §24](../../docs/protocols/instance-replication-v1.md) — hosts bridging that seam should set `asyncDataAdmissionFatal: true` on the edge factory). Test adapters and programmable controls are exported only from `@nomicore/ws-replication/testing`.
