/**
 * issue #422 类型面验收契约（test-d）——`listen: false` 配置联合 +
 * `nomicoreHubSessionHost` 服务签名锁定（契约 §12.1 T1–T4）。
 *
 * 交付形态：SA6 契约逐条形态逐字保留（`artifacts/sa6-issue422-contract-suite/` 为同源基底）。
 * 正向锁定 + `@ts-expect-error` 负控（未触发即 TS2578）；`--typecheck` 收集本文件。
 */
