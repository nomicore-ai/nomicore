 ...
 *   `{ ok:true, value:3, schema:null, truncated:false }`，且敌意迭代器函数**从未被调用**
 *   （`iteratorCalls === 0`——「绝不调用迭代协议」的载荷锚，可执行断言）；
 * - T2：Proxy 数组（get 陷阱对 `Symbol.iterator` 键抛错、索引键反射透传）→ 不抛、
 *   恰 `{ ok:true, value:3, schema:null, truncated:false }`；
 * - T3：值通道缺席吸收后尾段异态（`['absent-key', Symbol()]`）→ 不抛、恰
 *   `{ ok:true, value:undefined, schema:null, truncated:false }` 且 `'schema' in r === true`；
 * - 局部负控：合法 path `['count']` 不受守卫影响——schema 为投影文本字面量锚
 *   `# readData [count]\n\nnumber\n`（防守卫过拒；#364 起 schema 为 string，无冻结语义）。
 *
...
describe('issue #273 F-1（D8）：readData schema 通道敌意 path 规范化守卫——收敛 schema:null、零 throw、零敌意函数调用', () => {
  it('T1：重定义 Symbol.iterator 的敌意数组（索引读正常）→ 不抛、恰 {ok:true,value:3,schema:null,truncated:false}，敌意迭代器零调用', async () => {
    ...
    // ① 调用不抛且结果恰为收敛形状（读恒 ok、值语义与 doc-runtime 一致、schema null、无预算读 truncated=false）
  ...
  it('T2：Proxy 数组（Symbol.iterator 键 get 陷阱抛错、索引键反射）→ 不抛、恰 {ok:true,value:3,schema:null,truncated:false}', async () => {
    const runtime = await makeReadyRuntime();
  ...
  it('T3：值通道缺席吸收后尾段异态（["absent-key", Symbol()]）→ 不抛、恰 {ok:true,value:undefined,schema:null,truncated:false} 且 schema 键在场', async () => {
    const runtime = await makeReadyRuntime();
  ...
  it('局部负控：合法 path ["count"] 不受守卫影响——schema 等于投影文本字面量锚（红 #2 投影）', async () => {
    const runtime = await makeReadyRuntime();
    const r = runtime.readData(['count']);
    expectReadDataOk(r, {
      value: 3,
      schema: '# readData [count]\n\nnumber\n',
    });
    await runtime.close();
  });
});
